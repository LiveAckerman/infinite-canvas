...
+ [优化] **图片直接走 CDN，VPS 不再当中转**：设了 `IMAGE_CDN_BASE` 之后，`/api/images/:id` 直接 302 重定向到 Cloudflare R2 自定义域（如 `cdn.lijiwang.top`），浏览器直连最近的 CDN 节点拉图。国内用户访问历史图基本走 HK / 新加坡 POP，几十到几百毫秒；新图第一次仍要从 R2 EU 拉到 POP，但同一张图之后所有人访问都是缓存。

## v0.1.9 - 2026-06-12

+ [基础设施] **图片存储后端接 Cloudflare R2**：以前用户上传 / 生成的图片都落在服务器本地磁盘（`data/uploads/`），磁盘满了就麻烦。这版加了存储抽象层 —— 通过环境变量 `IMAGE_BACKEND=r2` 切到 R2 桶，DB 里只存对象 key，业务代码完全不感知是磁盘还是云端。本次发布默认仍 `local`，等迁移完成再切。
+ [运维] 新增一次性命令 `./server --migrate-images [--dry-run]`，把本地 `IMAGE_DIR` 下的图片整体搬到 R2，覆写已存在的对象，DB 行不动。

## v0.1.8 - 2026-06-12

+ [修复] **所有生图入口都改成后端任务驱动**：以前画布生图（节点 prompt 面板、角度生成）、画布右侧 AI 助手生图、以及角色工作台流水线 / 批量任务里每一步的生图，都是前端直发 + 等响应，刷新页面 / 切走 / 关 tab 就把图丢了。现在统一走 `/api/generations/run` —— 服务端起 worker 跑，前端只轮询；画布上 loading 节点甚至会把 `runningGenerationId` 一起保存，刷新画布后自动续上生成。生图记录里也能直接看到这些调用。

## v0.1.7 - 2026-06-12

+ [修复] **角色工作台并行模式刷新页面就丢进度**：以前并行卡片点「开始生成」是前端等响应的，刷新 / 切走再回来 loading 就没了、记录也不显示。现在改成后端任务驱动 —— 点了立刻在服务端入库 + 起后台任务，刷新页面卡片依然显示「生成中」并自动续上进度，生图记录里也马上能看到。和 /image 生图工作台一样的体验。

## v0.1.6 - 2026-06-11

+ [新增] **角色工作台「我的角色」面板可收起**：标题栏新增收起按钮，点一下侧栏缩成窄条，给右侧工作区让出空间；再点展开。状态会记住（参考生图工作台左侧记录面板的收起）。
+ [优化] **生图工作台：查看历史记录不再悄悄改你的默认参数**。之前点开一条旧记录会把你的默认「张数 / 尺寸 / 质量」改成那条记录的值并同步到账号，现在只在当前页面回填、不再改写默认偏好。
+ [修复] **生图工作台：重试 / 快速切换记录时，右侧结果偶尔串到别的记录上**。
+ [修复] **生图工作台：刷新或切回一条正在生成的记录时，「生成中 X 秒」会接着计时**（之前会一直停在「…」）。
+ [修复] **生图工作台：直接打开较早的生成记录链接不再误报「记录不存在」**（之前列表只取最近 100 条，更早的会误判）。
+ [优化] 生图工作台：把生成结果「加入素材」时存的是图床永久地址，避免极端情况下素材封面刷新后失效；删除当前记录后内部状态清理得更干净。
+ [修复] **角色工作台：批量启动 / 重做时若个别条目失败，会明确提示「N 条失败」**，不再只报成功数、让失败的悄悄留在原地。
+ [优化] **角色工作台：多开标签页 / 单步跑得很慢时，不会再出现同一条任务被重复执行、重复扣额度**（运行中会持续「报活」，只有真正卡死的任务才会被另一处接管）。
+ [优化] 角色工作台内部清理：去掉一处用不到的「停止」相关死代码、给调度器加了防重入保护、合并了几处重复的工具函数。

## v0.1.5 - 2026-06-02

+ [新增] **批量任务、执行流程支持批量删除**：角色工作台的「批量任务」列表和「流水线」列表都加了多选——勾选多条后点「删除选中」一次性删掉（带确认，关联图片按孤儿规则一并清理）。
+ [新增] **批次详情里可以单独重做某一条主条**：以前只有失败的主条能点「重试」，现在跑完的主条（成功 / 部分完成 / 失败）都能直接点「重做 / 重试」，只重跑这一条、不影响其它主条。

## v0.1.4 - 2026-06-02

+ [新增] **批量任务加「重做」按钮**：批次跑完后（成功 / 部分完成 / 失败都行），在批量任务列表卡片和批次详情页都能点「重做」，一键清掉现有产物、按现在的输入从头把整个批次（所有主条 + 后处理）重新跑一遍。点击有确认弹窗（会重新消耗额度）。

## v0.1.3 - 2026-06-02

+ [调整] 新用户注册赠送积分从 4 改为 10（仅影响之后新注册的账号，老用户不变）。

## v0.1.2 - 2026-06-02

+ [修复] **点按钮跳转时顶部加载进度条不出现**（批量任务「打开」、执行流程「打开」、画布「打开」、各种「返回」、个人中心、登录后跳转等）。原因：进度条只在点链接时自动出现，点按钮触发的跳转不会。统一加了一层 `useNav` 包装把这些跳转接上进度条；同页内的地址同步（如 /image 切到 /image/{id}）保持不闪进度条。

## v0.1.1 - 2026-06-02

+ [新增] 接入 umami 网页访问统计（自托管），用于看访问量；脚本在页面加载完成后异步加载，不影响打开速度。

## v0.1.0 - 2026-06-02

+ [优化] **生图改在服务器后台进行，刷新 / 切走都不丢**：在生图工作台点「开始生成」后，刷新页面、切到别的记录再切回、甚至关掉网页重开，正在生成的图都不会丢——回来能接着看到进度，直到全部出完。服务器重启了也会自动把没跑完的接着跑完。
+ [修复] **切换生成记录时，右侧结果偶尔不刷新、或串成别条记录的结果**（提示词、参考图变了但图没跟着变）。现在切到哪条就稳定显示哪条。
+ [修复] **偶发的「同一张图重复扣分」「任务一直转圈不结束」**：积分改成按实际出图张数扣，任务结束后顶栏余额自动刷新；某一张失败时可以单独点「重试」补出这一张。
+ [优化] **删除时顺带清理没用的图片**：管理后台删除公开素材 / 提示词 / 用户，以及画布删除时，会把关联、且没有被别处用到的图片一起清理掉（仍被别处引用的会保留）；删除确认按钮也会正确显示「加载中」。

## v0.0.42 - 2026-06-02

+ [优化] **批次详情页：失败主条卡片上直接「重试」+ 隐藏"复制 / 删除"按钮**：之前批量任务里某条主条失败了（比如某步上游报错），只能点「打开」进到 run 详情页里才能点「重新执行」，绕一步。现在批次详情卡片右下角直接显示「重试」按钮（仅 `status==="failed"` 时出现），点了清掉旧 steps、把 run 推回 queued，调度器会立刻接着跑——和详情页里「重新执行」完全等价。同时批次场景下原本无意义的「复制」「删除」按钮（点了只 toast「不可复制 / 不可删除」）直接不再渲染，避免误导。`PipelineRunCard` 新增四个可选 props：`onRetry / retrying / hideDuplicate / hideDelete`，默认全部 undefined，不影响 `/agents` 主页 pipeline 模式列表的现有行为。

+ [调整] **图片自动压缩参数调宽 + 页面上显式告知用户**：v0.0.41 给所有上传入口加了自动压缩，但用了 `maxSizeMB=2 / quality=0.85`，复杂细节图（人像 / 多色椅子）压完明显糊。现在改成 `maxSizeMB=4 / quality=0.95`（视觉无损）、小图阈值 200KB → 500KB（小图直接放行不动）、硬上限 5MB → 8MB；5 张参考图 × 4MB = 20MB 仍远低于上游 32MB。**同时把"会自动压缩"提示常驻到页面上**：① toast 文案改成「正在自动压缩…」（不再说"正在处理图片"，明确告诉用户在做什么）；② 公共组件 `ReferenceImagesField` 底部常驻一行小字「大于 500KB 的图会自动压缩到 4MB 内（长边 2048px），保持画质清晰」（可用 `showCompressHint={false}` 关闭），覆盖角色参考图 / 并行原图 / 素材库 / 批量模板等所有用它的场景；③ 角色头像上传的辅助文案追加「大图会自动压缩，放心传原图」。统一压缩文案常量 `COMPRESS_HINT_TEXT` 集中在 `lib/compress-image.ts`，参数调整后所有提示自动跟随。
...
+ [修复] **批次里打开主条 / 后处理详情后，点「返回」跳到 /agents 而不是回批次页**：run 详情页的返回 / 加载失败 / 删除后跳转都写死了 `/agents`。改成按 `run.batchId` 判断——属于某批次就回 `/agents/batches/{batchId}`，独立 run 才回 `/agents`。另外 post run 详情直接深链进来时，会把所属批次的 main runs 合并进 list cache，保证数据源解析和本页「重做」都能正常工作。

## v0.0.39 - 2026-06-01

+ [优化] **角色工作台「并行模式」原图开放到最多 9 张 + 四种上传方式**：之前并行模式每张卡片只能上传 1 张原图。现在每张卡片可上传最多 9 张（与模型 `/v1/images/edits` 的 `editsJSONReferenceLimit = 9` 对齐），且统一支持「点击上传（可多选）/ 读取剪切板 / Ctrl-V 粘贴 / 拖拽」四种方式，可一次加多张。生成时把「角色固定参考图」排在前、「卡片上传的原图」排在后拼成一组发给模型，相加超过 9 张时按「先角色后用户」截断并提示。多张原图会随卡片一起上云（跨设备恢复）。
+ [重构] **抽出公共参考图组件 `ReferenceImagesField`**（`web/src/components/reference-images-field.tsx`）：storageKey 数组进出，内部统一封装上传 / 剪切板 / 粘贴 / 拖拽四种添加方式、槽位换 / 删、`max` 上限截断、拖拽高亮、单选替换 / 多选追加。角色编辑弹窗的「固定参考图（最多 3 张）」与并行卡片的「原图（最多 9 张）」现在共用这一个组件，去掉了两边各自重复的一大段上传逻辑。（`/image` 工作台的参考图另有 ObjectURL / 画布截图回落那套模型，未纳入本次复用。）
+ [调整] **后端 `agent_workstation_cards.reference_key`（单图）→ `reference_keys`（JSON 数组）**：`model` 改 `ReferenceKeys []string`（`serializer:json`），`service` 校验逐张归属 + 上限 9 + 删除/孤儿扫描遍历数组。**这次加了旧数据兼容**：启动 `AutoMigrate` 后跑一次性、幂等的 `backfillAgentWorkstationReferenceKeys`——把老的单张 `reference_key` 迁进新数组列（仅在新列为空且老列非空时补），所以线上已挂在工作区里的并行卡片，升级后原图关联**不会丢**。全新库没有老列时该查询直接跳过；新代码不再写 `reference_key`，补过一次后不再命中。

## v0.0.38 - 2026-05-31

+ [调整] **单用户图片存储配额 500MB → 2GB**：线上 `nn` 用户 415 张图占满 500MB 后传不了图。磁盘 58G / 余 20G+，500MB 太紧。`UserImageQuotaBytes` 提到 2GB（admin 仍不限），错误文案改成跟常量联动（`fmt.Errorf`，改上限不用再手改字符串）。注：孤儿图片清理只能释放真正没引用的图（线上实测仅 8.9MB），用户的生图历史属于有用内容不会被自动删；要再省空间得用户自己删旧的生成记录 / 执行流程（会级联删图）。
+ [调整] **删除项目的确认弹窗统一提示「关联图片也会一并删除」**：用户侧各删除入口的后端其实早就级联清理了孤儿图（`CleanupImagesByKeysIfOrphan`），但确认弹窗文案没说清、个别还写反了。统一改成准确文案——
  - **修反了的**：执行流程删除原写「产物图本身不会被删」、批次删除原写「产物图保留在图床里」，但后端 `DeletePipelineRun` / `DeletePipelineBatch` 实际都级联删孤儿图。改成「关联的图片资源（原图、各步产物）也会一并删除（仍被别处引用的，比如已加入素材库的，会保留）」。
  - **补提示的**：画布删除（节点/连线/图片）、生图记录删除、我的素材删除、角色删除（头像/参考图）都加上「关联图片也会一并删除」的说明。
  - 统一口径：只删「未被别处引用的」图（orphan-only），同一张图若还挂在别的画布/记录/素材上则保留，不会删出破图。后端逻辑不变，本次只是把文案改准确。
  - 范围：管理后台的「公开素材 / 提示词」删除是 admin 维护的共享内容，`DeletePublicAsset` 目前不级联清理，未纳入本次（如需再单独处理）。
